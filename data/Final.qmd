...
model2 <- lm(sp ~ edge_distance + season,
                data = nest_boxes_clean)
...
The best model as determeined by Akaike's Information Criterion (AIC) is the one that compares if Swift Parrots are present depending on distance to forest edge and season. 

saturated model, the one that uses all the predictors: season and distance to forest edge.
